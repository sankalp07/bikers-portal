...
{	
	...
	$u_bday =  $_POST['u_bday'];
	$u_position = mysqli_real_escape_string($con,$_POST['u_position']);
	$u_address = mysqli_real_escape_string($con,$_POST['u_address']);
	$u_type = mysqli_real_escape_string($con,$_POST['u_type']);
	$u_pass = md5(mysqli_real_escape_string($con,$_POST['u_pass']));
	$u_gender = mysqli_real_escape_string($con,$_POST['u_gender']);
	$s_question = mysqli_real_escape_string($con,$_POST['s_question']);
	$s_ans = mysqli_real_escape_string($con,$_POST['s_ans']);
	$u_mobile = mysqli_real_escape_string($con,$_POST['u_mobile']);
	
	if(mysqli_query($con,"UPDATE users SET u_mobile='$u_mobile',s_ans='$s_ans',s_question='$s_question',u_gender='$u_gender',u_pass='$u_pass',u_address='$u_address',f_name='$f_name',l_name='$l_name',u_bday='$u_bday',u_position='$u_position',u_type='$u_type' WHERE u_email='$u_email'"))
		{
			?>
				...
				<div class="panel panel-default">
					...
					<form method="post">
						<fieldset>
							...
							<div class="form-group">
								Answer: <input class="form-control" placeholder="Answear" name="s_ans" type="text" value="<?php echo $userRow['s_ans']; ?>" required>
							</div>
							<div class="form-group">
								 <?php   
